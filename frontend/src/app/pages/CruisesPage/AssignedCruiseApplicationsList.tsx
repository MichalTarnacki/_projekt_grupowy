...
export function LinkWithStateDownloadApplication(
    props: LinkWithStateDownloadApplicationProps,
) {
    ...
    return (
        <a
            className={
                !props.disabled ? 'link-with-state' : 'link-with-state-disabled'
            }
            onClick={() => {
...
export default function AssignedCruiseApplicationsList(props: Props) {
    const canCurrentUserAccessCruiseApplication =
        CanCurrentUserAccessCruiseApplication();
    return (
        <>
            {props.cruiseApplicationsShortInfo.length == 0 && (
                <div>Brak zgłoszeń</div>
            )}
            {props.cruiseApplicationsShortInfo.map(
                (cruiseApplication, index: number) => (
                    <div
                        key={index}
