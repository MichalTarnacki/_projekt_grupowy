...
export default function useFormWrapper() {
            ...
            <>
                {errors[props.fieldName] && (
                    <ErrorMessageIfPresentNoContext
                        message={errors[props.fieldName]!.message as string}
                    />
                )}
            </>
        );
    ...
    const ComparePasswordWith = (field: string) => {
        return (value: FieldValues) =>
            value === form.getValues(field) || 'Hasła nie pasują do siebie';
    };
    ...
    const CommonSubmitButton = (props: { label: string }) => {
        return (
            <input
                className={
                    disabled ? 'login-common-submit-loading' : 'login-common-submit'
                }
                type="submit"
                disabled={disabled}
                value={props.label}
    ...
    const ReturnToLoginLink = () => {
        return (
            <input
                type={'submit'}
                className="login-common-submit"
                value={'Powrót do logowania'}
            />
        );
    };
