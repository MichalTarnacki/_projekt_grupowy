    ...
    : IFormsAFactory
{
    ...
    private async Task<Result> AddManagersTeam(FormA formA, FormADto formADto)
    {
        ...
        if (formADto.CruiseManagerId != currentUserId && formADto.DeputyManagerId != currentUserId)
            return Error.InvalidArgument("Użytkownik wysyłający zgłoszenie musi być kierownikiem lub jego zastępcą.");
        ...
        if (formADto.DeputyManagerId is not null &&
    ...
    private async Task<Result> AddResearchArea(FormA formA, FormADto formADto, CancellationToken cancellationToken)
    {
        ...
        var researchArea = await researchAreasRepository.GetById((Guid)formADto.ResearchAreaId, cancellationToken);
    ...
    private async Task<Result> AddFormAUgUnits(FormA formA, FormADto formADto, CancellationToken cancellationToken)
    {
